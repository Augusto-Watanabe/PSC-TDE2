...
Com essa ordem invertida, surge um ciclo de espera: T1 segura `LOCK_A` e espera `LOCK_B`, enquanto T2 segura `LOCK_B` e espera `LOCK_A`. Como nenhuma das duas libera o lock que ja possui, o programa trava.
...
A evidencia de travamento fica clara porque as threads nao chegam ao final na versao reproduzida. Na versao corrigida, a mesma ordem global de aquisicao elimina o ciclo de espera e permite que ambas concluam normalmente.